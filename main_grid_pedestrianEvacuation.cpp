...
#include <fstream>
#include <string>
#include "pedestrianEvacuation_grid_cell.hpp"

using namespace cadmium::celldevs;
using namespace cadmium::celldevs::example::pedestrianEvacuation;

std::shared_ptr<GridCell<PedestrianCell, double>> addGridCell(const coordinates & cellId, const std::shared_ptr<const GridCellConfig<PedestrianCell, double>>& cellConfig) {
	auto cellModel = cellConfig->cellModel;
	if (cellModel == "default" || cellModel == "pedestrianEvacuation") {
		return std::make_shared<GridPedestrianCell>(cellId, cellConfig);
	} else {
...
int main(int argc, char ** argv) {
	...
	auto model = std::make_shared<GridCellDEVSCoupled<PedestrianCell, double>>("pedestrianEvacuation", addGridCell, configFilePath);
	model->buildModel();
	...
	auto logger = std::make_shared<cadmium::CSVLogger>("pedestrian_grid_log.csv", ";");
